    ...
    <section className="py-16 sm:py-20 lg:py-24 bg-white relative">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-xs sm:text-sm font-bold text-orange-600 uppercase tracking-widest mb-2">Nuestros Pilares</h2>
          <h3 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 px-4">La base de una nación eficiente</h3>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          {pillars.map((pillar, index) => (
            <motion.div
              ...
              className="group p-6 sm:p-8 bg-slate-50 rounded-2xl border-2 border-slate-200 hover:border-orange-200 hover:shadow-xl hover:shadow-orange-900/5 transition-all duration-300 active:scale-[0.98]"
            >
              <div className="mb-5 sm:mb-6 p-3 sm:p-4 bg-white rounded-xl shadow-sm inline-block group-hover:scale-110 transition-transform duration-300 border border-slate-100">
                {pillar.icon}
              </div>
              <h4 className="text-lg sm:text-xl font-bold text-slate-900 mb-2">{pillar.title}</h4>
              <p className="text-orange-600 font-semibold mb-2 sm:mb-3 text-sm sm:text-base">{pillar.sub}</p>
